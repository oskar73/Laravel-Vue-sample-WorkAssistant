...
Route::name('employee.')->prefix('employee')->middleware('fw-only-whitelisted', 'passwordCheck')->group(function () {
    Route::get('/dashboard', [Employee\DashboardController::class, 'index'])->name('dashboard');
});

Route::name('employee.')->prefix('employee')->middleware('fw-only-whitelisted')->group(function () {
