...
$(function () {
    categoryTable = $('#all_area .datatable-all').DataTable(getCategoryTable())
})

function modalClearToggle(categoryId = '', name = '',  status = 1) {
    ...
    $('#addCategory_modal').modal('toggle')
}

function getCategoryTable() {
    let ajax = {
        url: '/account/product/category',
        type: 'get',
        data: {}
    }
...
$(document).on('click', '.edit_btn', function () {
    modalClearToggle($(this).data('id'), $(this).data('name'), $(this).data('status'))
})
...
function switchAction() {
    $.ajax({
        url: '/account/product/category/switch',
...
$('#addCategoryForm').on('submit', function (e) {
    ...
    //store url
    let url = '/account/product/category'
    let categoryId = $('#category_id').val()

    if (categoryId != '') {
        url = '/account/product/category/update/'+categoryId
    }
