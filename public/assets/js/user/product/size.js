...
$(function () {
    sizeTable = $('#all_area .datatable-all').DataTable(getSizeTable())
})

function modalClearToggle(sizeId = '', name = '', size_code = '', slug = '') {
    ...
    $('#addSize_modal').modal('toggle')
}

function getSizeTable() {
    let ajax = {
        url: '/account/product/size',
        type: 'get',
        data: {}
    }
...
$(document).on('click', '.edit_btn', function () {
    modalClearToggle($(this).data('id'), $(this).data('name'), $(this).data('size_code'), $(this).data('slug'))
})
...
function switchAction() {
    $.ajax({
        url: '/account/product/size/switch',
...
$('#addSizeForm').on('submit', function (e) {
    ...
    //store url
    let url = '/account/product/size'
    let sizeId = $('#size_id').val()

    if (sizeId != '') {
        url = '/account/product/size/update/'+sizeId
    }
