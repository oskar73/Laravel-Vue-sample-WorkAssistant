...
$(document).ready(function () {
  ...
  if (template) {
    template_obj.val(template)
    header_obj.val(header)
    footer_obj.val(footer)
    previewTemplate(template_img, template_slug)
  }
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      if (result.status === 0) {
        ...
        itoastr('success', 'Successfully created! Your website is ready now. Please visit website.')
...
$('#submit_domain').click(function () {
  ...
  if (domain === '') {
    ...
    $.ajax({
      url: '/account/website/connectDomain',
      type: 'post',
...
function loadCustom() {
  $.ajax({
    url: '/account/website/loadCustom',
    type: 'get',
    success: function (result) {
      console.log(result)
      if (result.status === 1) {
