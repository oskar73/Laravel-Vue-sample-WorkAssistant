...
$(document).ready(function () {
  ...
  const transcripts = urlParams.get('transcript');
  if(transcripts){
    $('.chat_list').hide()
    $('.chat_content').css('margin', '0px')
    $('.chat-form').hide()
    c_tab = urlParams.get('tab')
    c_item = urlParams.get('item')
    updateChatContent()
  }
})
...
function getDetail(id, tab) {
  $.ajax({
    url: '/admin/livechat/chatbox/getDetail',
...
function transcriptChat(id) {
  $.ajax({
    url: '/admin/livechat/chatbox/transcriptChat',
    data: { item: id },
    success: function (result) {
      if (result.status == 1) {
        itoastr('success', 'Successfully transferred!')
      }
    }
  })
}
function endGuestChat(id) {
  $.ajax({
    url: '/admin/livechat/chatbox/endGuestChat',
    data: { item: id },
    success: function (result) {
      console.log(result)
    }
...
function updateChatContent(scroll = 1) {
  $.ajax({
    url: `/admin/livechat/chatbox/getContent`,
...
function readMessage(tab = c_tab, item = c_item) {
  $.ajax({
    url: `/admin/livechat/chatbox/readMessage`,
...
function updateUnreadsFun(tab = c_tab, item = c_item) {
  $.ajax({
    url: `/admin/livechat/chatbox/updateUnreads`,
    data: { tab: tab, item: item },
    success: function (result) {
      console.log(result)
    }
...
function submitMsg(msgObj) {
  $.ajax({
    url: '/admin/livechat/chatbox/sendMessage',
    method: 'POST',
