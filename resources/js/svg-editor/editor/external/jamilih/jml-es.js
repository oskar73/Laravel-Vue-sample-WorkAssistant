...
const possibleOptions = [
  '$plugins', // '$mode', // Todo (SVG/XML)
  // '$state', // Used internally
  '$map' // Add any other options here
]
const NS_HTML = 'http://www.w3.org/1999/xhtml'
const hyphenForCamelCase = /-([a-z])/gu
const ATTR_MAP = {
  maxlength: 'maxLength',
  minlength: 'minLength',
  readonly: 'readOnly'
        ...
        */
        ...
        case '$define': {
          ...
          const getConstructor = (cnstrct) => {
            const baseClass =
              options && options.extends ? doc.createElement(options.extends).constructor : customizedBuiltIn ? doc.createElement(localName).constructor : window.HTMLElement
            /**
      ...
      case 'null':
        ...
        if (i === argc - 1) {
          ...
          return nodes.length <= 1
            ? nodes[0] // eslint-disable-next-line unicorn/no-fn-reference-in-iterator
        ...
        switch (arg) {
          ...
          default: {
            ...
            if (_getType(atts) === 'object' && atts.is) {
              const { is } = atts // istanbul ignore else
        ...
        if (atts.xmlns !== undefined) {
          ...
          elem = nodes[nodes.length - 1] = new win.DOMParser().parseFromString(
            new win.XMLSerializer()
              .serializeToString(elem) // Mozilla adds XHTML namespace
              .replace(' xmlns="' + NS_HTML + '"', replacer),
            'application/xml'
          ).documentElement // Todo: Report to plugins
...
jml.toJML = function (dom, { stringOutput = false, reportInvalidState = true, stripWhitespace = false } = {}) {
    ...
    switch (type) {
      case 1: {
        ...
        if (node.attributes.length) {
          set(
            [...node.attributes].reduce(function (obj, att) {
