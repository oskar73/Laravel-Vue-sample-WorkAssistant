...
function getCallbackSetter(funcName) {
  return function (...args) {
    ...
    const callbackID = this.send(funcName, args, function () {
      /* */
    ...
    setTimeout(
      (function (callbackID) {
        return function () {
          ...
          try {
            sameOriginWithGlobal = window.location.origin === that.frame.contentWindow.location.origin && that.frame.contentWindow.svgEditor.canvas
          } catch (err) {}
          ...
          if (sameOriginWithGlobal) {
            ...
            const {
              svgEditor: { canvas: svgCanvas }
            } = that.frame.contentWindow
            try {
