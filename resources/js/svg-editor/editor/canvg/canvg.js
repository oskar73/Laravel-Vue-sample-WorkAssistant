...
function build(opts) {
  ...
  svg.Element.path = class extends svg.Element.PathElementBase {
    constructor(node) {
      ...
      let d = this.attribute('d')
        .value // TODO: convert to real lexer based on https://www.w3.org/TR/SVG11/paths.html#PathDataBNF
          ...
          case 'a':
            while (!pp.isCommandOrEnd()) {
              ...
              let s =
                ...
                Math.sqrt((rx ** 2 * ry ** 2 - rx ** 2 * currp.y ** 2 - ry ** 2 * currp.x ** 2) / (rx ** 2 * currp.y ** 2 + ry ** 2 * currp.x ** 2))
              if (isNaN(s)) s = 0
  ...
  svg.Element.feColorMatrix = class extends svg.Element.ElementBase {
    constructor(node) {
      ...
      switch (
        this.attribute('type').valueOrDefault('matrix') // https://www.w3.org/TR/SVG/filters.html#feColorMatrixElement
      ) {
        case 'saturate': {
          const s = matrix[0]
          matrix = [
