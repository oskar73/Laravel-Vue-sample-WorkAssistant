...
import { importSetGlobal } from './external/dynamic-import-polyfill/importModule.js'
import {
  assignAttributes,
  cleanupElement,
  convertToPath,
...
import { getMatrix, hasMatrixTransform, isIdentity, matrixMultiply, rectsIntersect, snapToAngle, transformBox, transformListToTransform, transformPoint } from './math.js'
import { convertAttrs, convertToNum, convertUnit, getTypeMap, init as unitsInit, shortFloat } from './units.js'
import { isChrome, isFirefox, isGecko, isIE, isWebkit, supportsGoodTextCharPos, supportsNonScalingStroke } from './browser.js'
import { getTransformList, resetListMap, SVGTransformList as SVGEditTransformList } from './svgtransformlist.js'
...
import { initSnapper } from './snapper.js'
import { getConfig as appConfig } from './settings'
import eventBus from '@/public/eventBus'

let $ = jQueryPluginSVG(jQuery)
const { MoveElementCommand, InsertElementCommand, RemoveElementCommand, ChangeElementCommand, BatchCommand, UndoManager, HistoryEventTypes } = hstry
...
if (!window.rotate) {
  window.rotate = {}
  window.rotate.element = null
  window.rotate.tippy = null
}
...
class SvgCanvas {
  constructor(container, config) {
    const curConfig = {
      selectNew: true,
      // check if a text element is clicked or not.
      // it is useful to pop pu text control dialog.
      // if the click is first click, the pop up will be displayed soon,
      // but it's not a first clieck, the pop up will work only for dbl click.
      isTextClicked: false
    }
    ...
    $.extend(allProperties.text, {
      fill: curConfig.text && curConfig.text.fill,
      ...
      handleHistoryEvent(eventType, cmd) {
        ...
        if (eventType === EventTypes.BEFORE_UNAPPLY || eventType === EventTypes.BEFORE_APPLY) {
          ...
          if (cmdType === MoveElementCommand.type()) {
            ...
            if (cmd.elem && cmd.elem.tagName === 'title' && cmd.elem.parentNode.parentNode === svgcontent) {
              draw.identifyLayers()
            }
            const values = isApply ? cmd.newValues : cmd.oldValues
            // If stdDeviation was changed, update the blur.
            if (values.stdDeviation && cmd.elem) {
    ...
    const insertNewImage = (this.insertNewImage = function (src) {
      const width = '70%'
      const height = '70%'
     ...
     * @returns {void}
     */
    const importImage = (this.importImage = function (e) {
      e.stopPropagation()
      e.preventDefault()
    ...
    const getMouseTarget = (this.getMouseTarget = function (evt) {
      ...
      if (mouseTarget.id === 'backgroundRect') {
        return mouseTarget
      }
    ...
    this.selectAllInCurrentLayer = function () {
      ...
      if (currentLayer) {
        currentMode = 'select'
        selectOnly($(currentGroup || currentLayer).children(':not(#backgroundRect)'))
      }
    }
    ...
    ;(function () {
      ...
      const mouseDown = function (evt) {
        ...
        switch (currentMode) {
          case 'select':
            ...
            if (mouseTarget !== svgroot && mouseTarget.id !== 'backgroundRect') {
              ...
              function isMouseInsideRect(rect, mouseX, mouseY) {
                return mouseX >= rect.left && mouseX <= rect.right && mouseY >= rect.top && mouseY <= rect.bottom
              }

              const rect = $('#selectorParentGroup')[0].getBoundingClientRect()
              if (isMouseInsideRect(rect, evt.clientX, evt.clientY) && selectedElements[0].id !== 'backgroundRect') {
                break
              }
            ...
            if (selectedElements[0]) {
              ...
              const elements = canvas.getVisibleElements()
              const calculatedTargetBbox = {}

              const contentBbox = getBackgroundItem().getBoundingClientRect()
              const targetLeft = parseFloat(targetBbox.left / currentZoom - contentBbox.left / currentZoom)
              const targetTop = parseFloat(targetBbox.top / currentZoom - contentBbox.top / currentZoom)
              const targetRight = parseFloat(targetBbox.right / currentZoom - contentBbox.right / currentZoom)
              const targetBot = parseFloat(targetBbox.bottom / currentZoom - contentBbox.bottom / currentZoom)

              calculatedTargetBbox.left = targetLeft
              calculatedTargetBbox.top = targetTop
              calculatedTargetBbox.right = targetRight
              calculatedTargetBbox.bottom = targetBot
              calculatedTargetBbox.width = parseFloat(targetBbox.width / currentZoom)
              calculatedTargetBbox.height = parseFloat(targetBbox.height / currentZoom)
              calculatedTargetBbox.original_bbox = targetBbox

              // Set calculated bbox for target
              snapper[target.getAttribute('id')] = calculatedTargetBbox

              // Set bboxes for related visible elements
              for (const [key, visible] of Object.entries(elements)) {
                const calculatedVisibleBbox = {}
                const visibleBbox = visible.getBoundingClientRect()
                const visibleLeft = parseFloat(visibleBbox.left / currentZoom - contentBbox.left / currentZoom)
                const visibleTop = parseFloat(visibleBbox.top / currentZoom - contentBbox.top / currentZoom)
                const visibleRight = parseFloat(visibleBbox.right / currentZoom - contentBbox.right / currentZoom)
                const visibleBot = parseFloat(visibleBbox.bottom / currentZoom - contentBbox.bottom / currentZoom)
                calculatedVisibleBbox.left = visibleLeft
                calculatedVisibleBbox.top = visibleTop
                calculatedVisibleBbox.right = visibleRight
                calculatedVisibleBbox.bottom = visibleBot
                calculatedVisibleBbox.width = parseFloat(visibleBbox.width / currentZoom)
                calculatedVisibleBbox.height = parseFloat(visibleBbox.height / currentZoom)
                calculatedVisibleBbox.original_bbox = visibleBbox

                snapper[visible.getAttribute('id')] = calculatedVisibleBbox
              }
            }

            /**
             * Ungroup temporary group (multiselect)
             */
            const itemsForUngroup = getVisibleElements()
            const itemsSize = getSize(itemsForUngroup)

            // Check if click by not the <g> item
            if (!itemIsGroup(mouseTarget)) {
              if (itemsSize > 0) {
                for (let i = 0; i < itemsForUngroup.length; i++) {
                  const item = itemsForUngroup[i]
                  if (itemIsGroup(item) && item.childNodes.length > 0) {
                    canvas.ungroupSelectedElement(item)
                    canvas.removeFromSelection(selectedElements)
                  }
                }
                if (mouseTarget.id !== 'backgroundRect') {
                  eventBus.$emit('selected.changed')
                  document.getElementById('backgroundRect').style = ''
                }
              }
            }
            break
          ...
          case 'rotate':
            ...
            // Init tippy data for rotate placeholder
            setTimeout(() => {
              const element = document.getElementById('selectorGrip_rotate')
              const tippy = element._tippy

              // Show rotate placeholder
              tippy.show()

              // Save instances
              window.rotate.element = element
              window.rotate.tippy = tippy
            })
            break
          default:
      ...
      const mouseMove = function (evt) {
        ...
        switch (currentMode) {
          case 'select': {
            ...
            if (selectedElements[0] !== null) {
              ...
              dx = snapToObjects(dx, 'left')
              dy = snapToObjects(dy, 'top')
          ...
          case 'multiselect': {
            ...
            const elemsToAdd = []
            const newList = getIntersectionList()
            ...
            for (i = 0; i < len; ++i) {
              ...
              if (!selectedElements.includes(intElem) && intElem.id !== 'backgroundRect') {
          ...
          case 'resize': {
            ...
            if (_.includes(['se', 'sw', 'nw', 'ne'], currentResizeMode)) {
              if (!evt.shiftKey) {
                if (sx === 1) {
                  sx = sy
                } else {
            ...
            // Disable curve text resize (or text in group)
            if (selected.tagName === 'text' || selected.tagName === 'g') {
              if (sx === 1) {
                sx = sy
              } else {
                sy = sx
          ...
          case 'rotate': {
            ...
            if (!evt.shiftKey) {
              const snap = appConfig().snap.grid.step
            ...
            if (rotate.tippy) {
              ...
              rotate.tippy.setContent(Math.round(visibleAngle))
            }
            break
          }
          default:
        ...
        eventBus.$emit('editor-design-changed')
      } // mouseMove()
      ...
      const mouseUp = function (evt) {
        if (backgroundSelected) {
          addToSelection([document.getElementById('backgroundRect')])
          eventBus.$emit('background.selected')
        }
        if (evt.button === 2) {
          return
        }
        ...
        switch (currentMode) {
          ...
          case 'rotate': {
            ...
            // Hide tippy for rotate placeholder
            setTimeout(() => {
              const element = document.getElementById('selectorGrip_rotate')
              const tippy = element._tippy
        ...
        // Set select mode after insert some item in canvas
      ...
      const dblClick = function (evt) {
        ...
        if (tagName === 'text') {
          setTimeout(function () {
            eventBus.$emit('editor.panel.text.input.navigator', {
              force_show: true
            })
          }, 100)
        }
      ...
      const handleLinkInCanvas = function (e) {
        ...
        if (tagName === 'text') {
          if (curConfig.isTextClicked === false) {
            setTimeout(function () {
              eventBus.$emit('editor.panel.text.input.navigator', {
                force_show: true
              })
    ...
    const removeUnusedDefElems = (this.removeUnusedDefElems = function () {
      ...
      const defelemUses = ['mask', 'svg_mask']
      let numRemoved = 0
      ...
      const defelems = $(defs).find('linearGradient, radialGradient, filter, marker, svg, symbol, mask')
      i = defelems.length
     ...
     */
    this.svgCanvasToString = function (remainUnusedDefs) {
      ...
      if (elem) {
        ...
        if (elem.id === 'svgcontent') {
          ...
          for (let i = attrs.length - 1; i >= 0; i--) {
            ...
            if (attrVal !== '') {
              if (attrVal.startsWith('pointer-events') && attrVal.includes('mask')) {
                out.push(' style="mask: url(#svg_mask)" ')
                continue
              }
    ...
    const snapEvent = function (data) {
      eventBus.$emit('snap.lines.show', data)
    }

    const snapToObjects = function (offset, axis = 'left') {
      if (offset === 0) {
        return false
      }

      const distance = appConfig().snap.object.step
      const selectedItems = getSelectedElems()

      try {
        if (selectedItems.length === 1) {
          const target = selectedItems[0]

          // prevent offset for background rect
          if (target.getAttribute('id') === 'backgroundRect') {
            return 0
          }
          const targetBbox = snapper[target.getAttribute('id')]
          const excludeIds = ['snap-line-x', 'snap-line-y']

          // Snap group object to workarea
          if (!itemIsGroup(target)) {
            excludeIds.push('canvasBackground')
          }

          const elements = canvas.getVisibleElements(false, excludeIds)
          const indent = axis === 'left' ? 'width' : 'height'

          for (const [key, visible] of Object.entries(elements)) {
            if (target !== visible) {
              const visibleBbox = snapper[visible.getAttribute('id')]
              if (!visibleBbox) {
                continue
              }

              const targetIndent = targetBbox[indent]
              const targetLeft = targetBbox[axis] + offset
              const targetRight = targetBbox[axis] + targetIndent + offset
              const halfTargetIndent = targetIndent / 2

              const visibleIndent = visibleBbox[indent]
              const visibleLeft = visibleBbox[axis]
              const visibleRight = visibleLeft + visibleIndent

              const toLeftVisible = targetBbox[axis] - visibleLeft
              const toRightVisibleWithIndent = targetBbox[axis] - (visibleLeft + visibleIndent)
              const visibleCenter = visibleLeft + visibleIndent / 2
              const centerTarget = targetLeft + targetIndent / 2

              // Snap left target to left visible from right (x)
              // Snap top target to top visible from bot (y)
              // Snap left target to left visible from left (x)
              // Snap top target to top visible from top (y)
              if (_.inRange(targetLeft, visibleLeft + distance, visibleLeft - distance)) {
                const newOffset = Math.abs(toLeftVisible)

                offset = offset > 0 ? newOffset : -1 * newOffset
              ...
              // Snap left target to right visible from left (x)
              // Snap top target to bot visible from bot (y)
              // Snap left target to right visible from right (x)
              // Snap top target to bot visible from bot (y)
              if (_.inRange(targetLeft, visibleRight + distance, visibleRight - distance)) {
                const newOffset = Math.abs(toRightVisibleWithIndent)

                offset = offset > 0 ? newOffset : -1 * newOffset

                snapEvent({
                  gluingSide: axis === 'left' ? 'right' : 'bot',
                  visible,
                  target
                })
              }

              // Snap bot target to bot visible from left (x)
              // Snap right target to right visible from right (y)
              // Snap bot target to bot visible from right (x)
              // Snap right target to right visible from left (y)
              if (_.inRange(targetRight, visibleRight + distance, visibleRight - distance)) {
                const newOffset = Math.abs(toRightVisibleWithIndent + targetIndent)
              ...
              // Snap right target to left visible from right (x)
              // Snap bot target to top visible from right (y)
              // Snap right target to left visible from left (x)
              // Snap bot target to top visible from left (y)
              if (_.inRange(targetRight, visibleLeft + distance, visibleLeft - distance)) {
                const newOffset = Math.abs(toLeftVisible + targetIndent)
              ...
              // Comments
              if (_.inRange(targetLeft, visibleCenter + distance, visibleCenter - distance)) {
                const newOffset = Math.abs(visibleCenter - targetBbox[axis])

                offset = offset > 0 ? newOffset : -1 * newOffset

                snapEvent({
                  gluingSide: axis === 'left' ? 'center_y' : 'center_x',
                  visible,
                  target
                })
              }
              ...
              // Comments
              if (_.inRange(targetLeft + targetIndent, visibleCenter + distance, visibleCenter - distance)) {
                const newOffset = Math.abs(visibleCenter - targetBbox[axis] - targetIndent)
              ...
              if (_.inRange(centerTarget, visibleCenter + distance, visibleCenter - distance)) {
                const newOffset = Math.abs(visibleCenter - targetBbox[axis] - halfTargetIndent)
    ...
    const itemIsGroup = (this.itemIsGroup = function (item) {
      try {
        return item.tagName === 'g'
      } catch (err) {
        return false
      }
    })
     ...
     * @returns {Array}
     */
    const cleanItems = (this.cleanItems = function (items) {
      try {
        items = items || []
        const cleanItems = []
        items.forEach((item) => {
    ...
    const getSelectedElems = (this.getSelectedElems = function () {
      return cleanItems(selectedElements)
    })

    const groupForSelect = (this.groupForSelect = function () {
      let i = selectedElements.length

      // Check count selected items
      if (i < 2) {
        return
      }
      ...
      const batchCmd = new BatchCommand('Create group')
    ...
    const getSize = (this.getSize = function (items) {
      let count = 0
    ...
    this.setSvgString = function (xmlString, preventUndo) {
      try {
        ...
        content
          ...
          .each(function () {
            ...
            try {
              selectOnly([this], false)
              canvas.ungroupSelectedElement()
            } catch (error) {
              console.log('Cant ungroup svg element. ' + error)
            }
          })
        ...
        // Set workarea width
        if (parseInt(attrs.height) > this.contentW) {
          this.contentW = attrs.width
        }

        // Set workarea height
        if (parseInt(attrs.height) > this.contentH) {
        ...
        // Clean selected elements (for array count)
        selectedElements = []
      ...
      initSnapper(getBackgroundItem(), currentZoom)

      // add background rect
      if (this.getOnlySvgElement().find('#backgroundRect').length === 0) {
        const rect = this.addSVGElementFromJson({
          element: 'rect',
          attr: {
            id: 'backgroundRect',
            width: '100%',
        ...
        $('#svgcontent .only_svg_content').prepend(rect)
      }

      this.getBackgroundElement = () => {
        return document.getElementById('backgroundRect')
      }

      // check if mask exist
      const svgMask = this.findSvgMask()
      if (svgMask) {
        const maskSvg = $(svgMask).find('svg')
        if (maskSvg && maskSvg.length > 0) {
          maskSvg[0].style.fill = '#ffffff'
          if (svgMask.html()) {
            this.addMaskToCanvasBackground(svgMask.html())
          }
        }
      }
    ...
    this.getOnlySvgElement = () => {
      return $('#svgcontent .only_svg_content')
    }
    ...
    this.findSvgMask = () => {
      const defs = this.findDefs()
      let mask = $(defs).find('mask')
      if (mask.length === 0) {
        mask = this.addSVGElementFromJson({
          element: 'mask',
          attr: {
            id: 'svg_mask',
            x: 0,
    ...
    this.addMaskToCanvasBackground = (maskSvgHtml) => {
      const backgroundCanvas = this.getBackgroundItem()
      let bgDefs = backgroundCanvas.getElementsByTagNameNS(NS.SVG, 'defs')
      if (bgDefs.length > 0) {
        bgDefs = bgDefs[0]
      } else {
        bgDefs = backgroundCanvas.ownerDocument.createElementNS(NS.SVG, 'defs')
        if (backgroundCanvas.firstChild) {
          // first child is a comment, so call nextSibling
          backgroundCanvas.insertBefore(bgDefs, backgroundCanvas.firstChild.nextSibling)
          // svgElement.firstChild.nextSibling.before(defs); // Not safe
        } else {
          backgroundCanvas.append(bgDefs)
        }
      }
      let bgMask = $(bgDefs).find('mask')
      if (bgMask.length === 0) {
        bgMask = this.addSVGElementFromJson({
          element: 'mask',
          attr: {
            id: 'bg_mask',
            x: 0,
        ...
        bgDefs.append(bgMask)
      }
      if (maskSvgHtml) {
        const bgMaskObject = $('<div/>').html(maskSvgHtml).find('svg')
        $(bgMask).html(bgMaskObject[0])
        bgMaskObject[0].style.fill = '#ffffff'
        bgMaskObject[0].setAttribute('id', 'bg_mask')
        backgroundCanvas.setAttribute('mask', 'url(#bg_mask)')
      } else {
        $(bgMask).html('')
        backgroundCanvas.removeAttribute('mask')
      }
    }

    this.addMaskToCanvas = (maskSvg) => {
      if (maskSvg) {
        const maskObject = $('<div/>').html(maskSvg).find('svg')
        if (maskObject) {
          maskObject[0].style.fill = '#ffffff'
          maskObject[0].setAttribute('id', 'svg_mask')

          // add mask to design
          const svgMask = this.findSvgMask()
          $(svgMask).html(maskObject[0])
          const svgElement = this.getOnlySvgElement()
          svgElement[0].style.mask = 'url(#svg_mask)'

          // add mask to canvas background
          this.addMaskToCanvasBackground(maskSvg)
        }
      } else {
        this.removeMask()
      }
    ...
    this.removeMask = () => {
      ...
      $(svgMask).html('')
      const svgElement = this.getOnlySvgElement()
      svgElement[0].style.mask = null
      svgElement[0].setAttribute('mask', null)
      this.addMaskToCanvasBackground(null)
    }
    ...
    this.getBackgroundColor = function () {
      const background = getBackgroundItem()

      if (background) {
        const rectangle = background.children[0]
        const attributes = rectangle.attributes
     ...
     * @param {"fill"|"stroke"|"background"} type - String indicating "fill" or "stroke" to apply to an element
     * @returns {void}
     */
    const setGradient = (this.setGradient = function (type, preventUndo) {
      ...
      if (type === 'background') {
        ...
        const rectangle = background.children[0]
        // Set color
        rectangle.attributes.fill.value = 'url(#' + grad.id + ')'
      } else {
        canvas.setColor(type, 'url(#' + grad.id + ')', preventUndo)
      }
    })
     ...
     * @param {"fill"|"stroke"|"background"} type - String with "fill" or "stroke"
     * @param {module:jGraduate.jGraduatePaintOptions} paint - The jGraduate paint object to apply
     * @returns {void}
     */
    this.setPaint = function (type, paint, preventUndo) {
      // make a copy
     ...
     * @returns {void}
     */
    this.setStrokePaint = function (paint, preventUndo) {
      this.setPaint('stroke', paint, preventUndo)
    }
     ...
     * @returns {void}
     */
    this.setFillPaint = function (paint, preventUndo) {
      this.setPaint('fill', paint, preventUndo)
    }

    this.setBackgroundPaint = function (paint) {
      let grad = paint[paint.type]
      // find out if there is a duplicate gradient already in the defs
      const duplicateGrad = findDuplicateGradient(grad)
      const defs = findDefs()
      // no duplicate found, so import gradient into defs
    ...
    this.getStrokeColor = function (element) {
      const elem = element || selectedElements[0]
      const defaultValue = '#FFFFFF'

      try {
        const value = elem.attributes.stroke.value
    ...
    this.getLetterSpacing = function (elem) {
      const element = elem || selectedElements[0]
      const defaultValue = 0

      try {
        let attribute = 'letter-spacing'

        // Change attribute for Firefox
        if (isFirefox()) {
    ...
    this.setLetterSpacing = function (val) {
      ...
      changeSelectedAttribute(attribute, val)
    }
    ...
    this.copySelectedElements = function () {
      const data = JSON.stringify(selectedElements.map((x) => getJsonFromSvgElement(x)))
     ...
     * @returns {void}
     */
    this.ungroupSelectedElement = function (group) {
      let g = group || selectedElements[0]

      if (!g) {
        return
      }

      if (g.classList.contains('do-not-ungroup')) {
        return
      }
    ...
    this.moveUpDownSelected = function (dir) {
      ...
      if (!closest || (dir === 'Down' && closest.id === 'backgroundRect')) {
        return
      }
    ...
    this.alignSelectedElements = function (type, relativeTo) {
      ...
      for (let i = 0; i < len; ++i) {
        ...
        switch (relativeTo) {
          case 'smallest':
            if (
              ((type === 'l' || type === 'c' || type === 'r') && (curwidth === Number.MIN_VALUE || curwidth > bboxes[i].width)) ||
    ...
    const getBackgroundItem = (this.getBackgroundItem = () => {
    ...
    this.setBackground = function (color, url) {
      ...
      // Set color
      rectangle.attributes.fill.value = color
      ...
      if (color === 'chessboard') {
        if (!bgPattern) {
          ...
          assignAttributes(div, {
            style:
              'width:100%;height:100%;background-image:url(data:image/gif;base64,R0lGODlhEAAQAIAAAP///9bW1iH5BAAAAAAALAAAAAAQABAAAAIfjG+gq4jM3IFLJgpswNly/XkcBpIiVaInlLJr9FZWAQA7);'
          })
