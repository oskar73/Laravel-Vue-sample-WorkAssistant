...
export default defineComponent({
  ...
  created() {
    ...
    if (!cart.items.length) {
      this.$router.replace('/checkout/canceled')
    }
    this.cart = { ...cart }
    cart.clear()
  ...
  methods: {
    checkoutSuccessHandler() {
      checkoutSuccess(this.$route.query.session, this.cart).then((res) => {
...
<template>
  <page-layout>
    <main v-if="success" class="tw-container tw-p-12 tw-mx-auto">
      ...
      <div v-if="cart.items.length" class="tw-container tw-py-12 tw-mx-auto">
