...
import themeMixin from '@/section-builder/mixins/themeMixin'
...
export default defineComponent({
  ...
  methods: {
    addCart() {
      ...
      this.cart.addItem(this.product, parseInt(this.qnt), options, this.price)
    },
    ...
    checkProperty() {
      ...
      if (!this.product.continue_selling && this.product.quantity < this.qnt) {
        if (this.product.quantity < this.qnt) {
          toast.error(`Only ${this.product.quantity} products available!`)
          return false
        }
        const total = this.cart.items.reduce((sum, item) => (sum += item.quantity), 0)
        if (total < this.qnt) {
          toast.error("Oops! You already have this product in your cart. The quantity you've selected exceeds the available quantity. Please adjust the quantity accordingly.")
          return false
        }
      }
            ...
            <div>
              ...
              <div class="tw-my-3">
                <div class="tw-text-lg tw-font-bold">
                  Quantity<span v-if="!product.continue_selling" class="tw-text-sm tw-font-semibold"> ({{ product.quantity }} products available)</span>
                </div>
