<template>
  <div class="bz-section-container bz-sec--contact-form-4-root" :class="{ [breakPoint]: true }">
    <bz-background :setting="background" :size="sectionSize">
      <bz-container>
        <bz-card :background-color="cardBackgroundColor" class="bz-card-root bz-overflow-hidden">
          <div class="tw-flex tw-flex-col lg:tw-flex-row tw-gap-6" :class="{ 'lg:tw-flex-row-reverse tw-flex-col-reverse': setting.alignment === 'right' }">
            <div class="tw-w-full lg:tw-p-12 tw-p-4">
              <bz-alignment :alignment="setting.layouts.alignment" :stretch="false">
                <bz-title v-if="setting.elements.title" v-model="data.elements.title" :background-color="cardBackgroundColor" />

                <bz-subtitle v-if="setting.elements.subtitle" v-model="data.elements.subtitle" :background-color="cardBackgroundColor" />

                <bz-text v-if="setting.elements.description" v-model="data.elements.description" :background-color="cardBackgroundColor" />
              </bz-alignment>
            ...
            <div class="tw-w-full max-lg:tw-hidden">
...
import BzInput from '../../components/page/BzInput.vue'
...
<style lang="scss">
.bz-sec--contact-form-4-root {
  @import 'styles';
  .bz-card-root {
    //border-top-left-radius: 1em;
    //border-bottom-left-radius: 1em;
    border-radius: 1em;
    box-shadow: 0 0.25rem 0.5rem -0.125rem #00000012, 0 1rem 1.5rem -0.5rem #00000012 !important;
  }
}
</style>
