    ...
    <bz-background :setting="background" :size="sectionSize">
      <bz-container>
        ...
        <div class="tw-flex tw-justify-center">
          <button v-if="isBuilder" class="tw-bg-blue-500 tw-text-white tw-rounded tw-shadow tw-px-4 tw-py-1" @click.prevent="handleAddItem()">Add Blog</button>
          <a @click="goToPage()" class="cursor-pointer tw-bg-blue-500 tw-text-white tw-rounded tw-shadow tw-px-4 tw-py-1" v-if="blogPosts.length && !isBuilder">View</a>
        </div>
      </bz-container>
    </bz-background>
  </div>
