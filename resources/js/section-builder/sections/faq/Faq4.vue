<template>
  <div class="bz-section-container bz-sec--faq4-root" :class="{ [breakPoint]: true }">
      ...
      <bz-container>
        ...
        <bz-items v-model="data.items" :cols="setting.column" :shadow="false" :enable-sort="edit">
          <template v-slot="{ item }">
            <div class="tw-border-b border-2 tw-mb-4" :style="{ borderColor: headingColor }">
              <bz-title v-model="item.question" />
            </div>
            <bz-text v-model="item.answer" />
          </template>
        </bz-items>
      </bz-container>
    </bz-background>
...
export default {
  components: { BzItems, BzTitle, BzContainer, BzBackground },
  mixins: [sectionMixin]
}
</script>
