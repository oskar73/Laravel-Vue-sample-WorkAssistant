    ...
    <div class="menu-wrap">
      <div class="bz-el-menu">
        <template v-for="(page, index) of allPages" :key="index">
          <router-link v-if="showMenuItem(page) && index <= showMenuIndex" :to="page.url">
            <div ref="menuItems">
        ...
        <div v-if="showMenuIndex < allPages.length - 1" class="more-menu">
          ...
          <div v-click-outside="handleOutsideClick" class="dropdown" :class="{ open: openDropDown }">
            <template v-for="(page, index) of allPages">
              <div v-if="showMenuItem(page) && index > showMenuIndex" ref="menuItems" :key="index" @click.stop.prevent="setActivePage({ index, type: page.type })">
                <slot :page-name="page.name" :active="isActiveMenu(page.url)" />
              </div>
            </template>
          </div>
