...
@section('content')
    <div class="m-portlet m-portlet--mobile tab_area area-active md-pt-50">
        ...
        <div class="m-portlet__body">
            <form action="{{route('user.product.update', ['id' => $product->id])}}" id="submit_form">
                @csrf
                <div class="container">
                    <div class="row mb-5">
                        <div class="col-md-7">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control m-input--square minh-50" name="name" id="name" placeholder="Name" value="{{old('name', $product->name)}}">
                            ...
                            <div class="form-row">
                                <div class="form-group col-md-4">
                                    ...
                                    <div class="input-group mb-2">
                                        ...
                                        <input type="number" class="form-control" id="price" name="price" min="1" max="100000" placeholder="Price" value="{{old('name', $product->price)}}">
                                        <div class="form-control-feedback error-price"></div>
                                    </div>
                                </div>
                                <div class="form-group col-md-4">
                                    <label for="quantity">Quantity</label>
                                    <input type="number" class="form-control" id="quantity" name="quantity" min="1" max="100000" placeholder="Quantity" value="{{old('name', $product->quantity)}}">
                                ...
                                <div class="form-group col-md-4">
                                    ...
                                    <select id="unit_id" name="unit_id" class="form-control">
                                        ...
                                        @foreach ($units as $unit)
                                            <option value="{{ $unit->id }}" {{old('name', $product->unit_id) == $unit->id ? 'selected':''}}>{{ $unit->name }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="form-group">
                                ...
                                <select id="category_id" name="category_id" class="form-control">
                                    ...
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->id }}" {{old('name', $product->category_id) == $category->id ? 'selected':''}}>{{ $category->name }}</option>
                                    @endforeach
                                </select>
...
@section('script')
    <script>
        ...
        var additionalPrices = @json($product->additionalPrices);
        var sizes = @json($sizes);
        var colors = @json($colors);
        @if($product)
            window.thumbNailUrl = '{{$product->getFirstMediaUrl('thumbnail')}}';
        @endif
    </script>
    <script src="{{asset('assets/js/user/product/productEdit.js')}}"></script>
@endsection
