        ...
        <ul class="m-subheader__breadcrumbs m-nav m-nav--inline">
            ...
            <li class="m-nav__item">
                <a href="{{ route('user.appointment.index') }}" class="m-nav__link">
                    <span class="m-nav__link-text">Site Appointment</span>
                </a>
            </li>
            <li class="m-nav__separator">/</li>
            <li class="m-nav__item">
                <a href="{{ route('user.appointment.category.index') }}" class="m-nav__link">
...
@section('content')
    ...
    <div class="m-portlet m-portlet--mobile tab_area area-active md-pt-50"  id="detail_area">
        <div class="m-portlet__body">
            <form action="{{route('user.appointment.category.store')}}" id="submit_form">
                @csrf
                    ...
                    <div class="row mb-5">
                        <div class="col-md-6 offset-md-3">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control selectpicker" id="web_id" name="web_id">
                                    @foreach($userWebsites as $webiste)
                                        <option value="{{$webiste->id}}" @if($category->web_id == $webiste->id) selected @endif>{{$webiste->domain}}</option>
                                    @endforeach
                                </select>
                                <div class="form-control-feedback error-website_id"></div>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="row">
                                ...
                                <div class="col-6">
                                    <div class="mt-3 text-right">
                                        <a href="{{route('user.appointment.category.index')}}" class="m-1 btn m-btn-square btn-outline-info m-btn m-btn--custom">Back</a>
    ...
    @include("components.admin.blockDateModal", ['web_id'=>$category->web_id])
@endsection
@section('script')
    <script src="{{asset('assets/js/user/appointment/own/categoryCreate.js')}}"></script>
    <script>var id = "{{$category->id}}", type="category";</script>
    <script src="{{ asset('assets/vendors/calendar/fullcalendar.js') }}"></script>
    <script src="{{asset('assets/js/user/appointment/own/blockDate.js')}}"></script>
@endsection
