            ...
            <form class="domain-search" id="search-form">
                ...
                <div class="container">
                    <div class="text-center mb-3">
                        <a href="javascript:void(0);" class="btn m-btn--square btn-info m-btn m-btn--custom">Register Domain</a>
                        <a href="{{route('admin.domain.connect')}}" class="btn m-btn--square  btn-outline-info m-btn m-btn--custom">Connect My Domain</a>
                        {{--                        <a href="{{route('admin.domain.transfer')}}" class="btn m-btn--square  btn-outline-info m-btn m-btn--custom">Transfer</a>--}}
                    </div>
                    ...
                    <div class="text-center">
                        <a href="javascript:void(0);" class="btn btn-primary mt-2 hover-box loadMoreBtn d-none">Load More...</a>
                    </div>
                </div>
            </form>
        </div>
    ...
    <div class="m-portlet m-portlet--mobile tab_area" id="contacts_area">
        <div class="m-portlet__head">
            <div class="m-portlet__head-caption">
                <div class="m-portlet__head-title">
                    <h3 class="m-portlet__head-text">
                    Domain Name: <b><span class="duration_domain">{{Session::get("pickDomain")?? ''}}</span></b>&nbsp;&nbsp; Duration: <b><span class="duration_num">{{Session::get("duration")?? '0'}} Years</span></b>
                    </h3>
                </div>
            </div>
            <div class="m-portlet__head-tools">
        ...
        <div class="m-portlet__body">
           <div class="contacts_result">
                <form action="{{route('admin.domain.setContact')}}" method="POST" id="contactForm">
                    @csrf
                    <div class="container">
                        <div class="row">
                            <div class="col-md-6">
                            ...
                            <div class="col-md-6">
                                <select name="contact" id="contact" class="contact selectpicker" data-width="100%">
                                    ...
                                    @foreach(user()->domainContacts()->select("contactName", "firstName", "lastName", "email", "address1", "address2", "city", "state", "postalCode", "country", "phoneCode", "phoneNumber")->get() as $contact)
                                        <option value="{{$contact->id}}" data-contact="{{$contact}}">{{$contact->contactName}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <div class="form-group m-form__group">
                                    <label class="m-checkbox m-checkbox--state-success mt-2">
                                        <input type="checkbox" name="saveThis" id="saveThis"> Remember this address for later.
                                        <span></span>
                                    </label>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group m-form__group d-none contact_name_area">
                                    <input type="text" class="form-control m-input m-input--square" name="contactName" placeholder="Name">
                                    <div class="form-control-feedback error-contactName"></div>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group m-form__group text-right">
                                    <button type="submit" class="btn btn-success formSmtBtn tw-bg-green-600">Submit</button>
                                </div>
                            </div>
                        </div>
                    </div>
...
@section('script')
    ...
    <script src="{{asset('assets/js/admin/domain/search.js')}}"></script>
@endsection
