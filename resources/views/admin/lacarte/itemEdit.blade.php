...
@section('breadcrumb')
        ...
        <ul class="m-subheader__breadcrumbs m-nav m-nav--inline">
            ...
            <li class="m-nav__item">
                <a href="" class="m-nav__link">
                    <span class="m-nav__link-text">Lacarte</span>
                </a>
            </li>
            <li class="m-nav__separator">/</li>
            <li class="m-nav__item">
    ...
    <div class="col-md-6 text-right">
        <a href="{{route('admin.lacarte.item.index')}}" class="btn btn-outline-info m-btn m-btn--custom m-btn--square">Back</a>
    </div>
@endsection
...
@section('content')
    <div class="tabs-wrapper">
        <ul class="tab-nav">
            <li class="tab-item"><a class="tab-link tab-active" data-area="#all" href="#/all">Lacarte Detail</a></li>
            <li class="tab-item"><a class="tab-link" data-area="#meeting" href="#/meeting">Meeting and Attach Form</a></li>
        </ul>
    </div>
    <div class="m-portlet m-portlet--mobile tab_area area-active md-pt-50" id="all_area">
        <div class="m-portlet__body">
            <form action="{{route('admin.lacarte.item.update', $item->id)}}" id="submit_form" method="post" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-md-6">
    ...
    @include("components.admin.common.meetingForm", ['action'=>route('admin.lacarte.item.updateMeetingForm', $item->id)])
@endsection
@section('script')
    <script type="text/javascript" src="{{s3_asset('vendors/cropper/cropper.js')}}"></script>
    <script>
        var category = "{{$item->category_id}}",
            item_id = "{{$item->id}}", maxImageSize = "{{config('custom.variable.max_image_size')}}";
        @if($item->getFirstMediaUrl("thumbnail"))
            window.thumbNailUrl = '{{$item->getFirstMediaUrl("thumbnail")}}';
        @endif
    </script>
    <script src="{{asset('assets/js/admin/lacarte/itemEdit.js')}}"></script>
