<div class="table-responsive">
    <table class="table table-hover ajaxTable datatable {{ $selector }}">
        ...
        @foreach ($templates as $key => $template)
            <tr>
                ...
                <td>
                    ...
                    <a class="text-black hover-none"
                       href="{{ route('user.template.item.editPages', $template->id) }}" target="_blank"><i
                ...
                <td>
                    <a href="javascript:void(0);" class="btn btn-info btn-sm m-1 p-2 m-btn m-btn--icon palettes"
                       data-id="{{ $template->id }}">
                        <span>Palettes</span>
                    </a>
                    <a href="{{ route('user.template.item.editPages', $template->id) }}"
                    ...
                    <a href="" data-url="{{ route('user.template.item.edit', $template->id) }}" data-toggle="modal"
            ...
            <tr>
                <td colspan="7" class="palette-row">
                    <div id="palettes_{{ $template->id }}" class="palettes-area">
                        @foreach ($template->themes as $theme)
                            <div class="text-left">{{ $theme['name'] ?? '' }}</div>
                            <div class="theme-colors">
                                <div class="theme-color-item">
                                    <div style="background: {{ $theme['backgroundColor'] }}"></div>
                                </div>
                                <div class="theme-color-item">
                                    <div style="background: {{ $theme['buttonColor'] }}"></div>
                                </div>
                                <div class="theme-color-item">
                                    <div style="background: {{ $theme['socialIconColor'] }}"></div>
                                </div>
                                <div class="theme-color-item">
                                    <div style="background: {{ $theme['headingColor'] }}"></div>
                                </div>
                                <div class="theme-color-item">
                                    <div style="background: {{ $theme['boxColor'] }}"></div>
                                </div>
                                <div class="theme-color-item">
