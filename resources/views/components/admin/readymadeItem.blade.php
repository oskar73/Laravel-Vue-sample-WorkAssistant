            ...
            <tr>
                ...
                <td>
                    @foreach($item->approvedPrices as $price)
                        ${{formatNumber($price->price)}} @if($price->recurrent) /{{periodName($price->period, $price->period_unit)}}  (Recurrent) @else  {{"( Onetime )"}} @endif
                        <br>
                    @endforeach
                </td>
                <td>
                ...
                <td>
                    <a href="{{route('admin.readymade.item.edit', $item->id)}}" class="btn btn-outline-info btn-sm m-1	p-2 m-btn m-btn--icon edit_btn">
                        <span>
