    ...
    <table class="table table-hover ajaxTable datatable {{$selector}}">
        ...
        <tbody>
            @foreach($headers as $header)
                <tr>
                    <td><input type="checkbox" class="checkbox" data-id="{{$header->id}}"></td>
                    <td>{{$header->name}}</td>
                    <td>{{$header->name}}</td>
                    <td>{{$header->templates_count}}</td>
                    <td>
                        @if($header->status===1)
                    ...
                    <td>
                        <a href="javascript:void(0);" class="tab-link btn btn-outline-info btn-sm m-1	p-2 m-btn m-btn--icon edit_btn" data-id="{{$header->id}}">
                            <span>
