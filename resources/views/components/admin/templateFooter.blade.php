<div class="table-responsive">
    <table class="table table-hover ajaxTable datatable {{$selector}}">
        ...
        @foreach($footers as $footer)
            <tr>
                <td><input type="checkbox" class="checkbox" data-id="{{$footer->id}}"></td>
                <td>{{$footer->name}}</td>
                <td>{{$footer->name}}</td>
                <td>{{$footer->templates_count}}</td>
                <td>
                    @if($footer->status===1)
                ...
                <td>
                    <a href="javascript:void(0);" class="tab-link btn btn-outline-info btn-sm m-1	p-2 m-btn m-btn--icon edit_btn"  data-id="{{$footer->id}}">
                        <span>
