    ...
    <table class="table table-hover ajaxTable datatable {{$selector}}">
        ...
        @foreach($items as $item)
            <tr>
                ...
                <td>
                    @if($item->slashed_price!==null) <span class="slashed_price_text">${{formatNumber($item->slashed_price)}}</span>  <br>@endif
                ...
                <td>
                    <a href="{{route('admin.lacarte.item.edit', $item->id)}}" class="btn btn-outline-info btn-sm m-1	p-2 m-btn m-btn--icon edit_btn">
                        <span>
