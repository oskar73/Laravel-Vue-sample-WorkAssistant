    ...
    <table class="table table-hover ajaxTable datatable {{$selector}}">
        ...
        @foreach($items as $item)
            <tr>
                ...
                <td>
                    ...
                    @else
                        @forelse($item->modules as $module)
                            <x-layoutItems.c-badge name="{{$module->name}}"/>
                        @empty
                            No modules
                        @endforelse
                    @endif
                ...
                <td>
                    <a href="{{route('admin.tutorial.item.detail', $item->id)}}" class="btn btn-outline-success btn-sm m-1	p-2 m-btn m-btn--icon edit_btn">
                        <span>
                            <i class="la la-eye"></i>
                            <span>Detail</span>
                        </span>
                    </a>
                    <a href="{{route('admin.tutorial.item.edit', $item->id)}}" class="btn btn-outline-info btn-sm m-1	p-2 m-btn m-btn--icon edit_btn">
                        <span>
