...
@section('title', 'Bizinabox Module - ' . $item->name)

@section('seo')
    <link rel="canonical" href="{{ config('app.url') }}/modules/{{ $item->slug }}">
@endsection
    ...
    <div class="container">
        <ol class="breadcrumb mt-3">
            ...
            @if($item->category->category->name!==null)
                <li class="breadcrumb-item"><a href="{{route('module.index')}}#/{{$item->category->category->slug}}">{{$item->category->category->name}}</a></li>
            @endif
            <li class="breadcrumb-item active"><a href="{{route('module.index')}}#/{{$item->category->slug}}">{{$item->category->name}}</a></li>
        </ol>
            ...
            <div class="col-md-7">
                <div class="product-detail">
                   ...
                   <div class="margin-20px-bottom white-space-pre-line">
                    ...
                    <a href="javascript:void(0);" class="btn btn-success addToCartBtn">
                        Add to Wishlist
                    </a>

                    <div class="alert alert-success mt-3" role="alert">
                        Ready to purchase? Choose a  <a href="{{route('package.index')}}" class="text-primary">package</a>.
                    </div>
...
{{--                        </div>--}}
        ...
        <x-front.reviewForm type="module" :id="$item->id"></x-front.reviewForm>
...
@section('script')
    ...
    <script>var type = "module", model_id="{{$item->id}}"; </script>
    <script src="{{asset('assets/js/front/module/detail.js')}}"></script>
@endsection
