...
@section('title', 'Bizinabox Plugin - ' . $item->name)

@section('seo')
    <link rel="canonical" href="{{ config('app.url') }}/plugins/{{ $item->slug }}">
@endsection
...
@section('content')
    ...
    <div class="container">
        <ol class="breadcrumb mt-3">
            <li class="breadcrumb-item"><a href="{{route('plugin.index')}}">Plugin</a></li>
            @if($item->category->category->name!==null)
                <li class="breadcrumb-item"><a href="{{route('plugin.index')}}#/{{$item->category->category->slug}}">{{$item->category->category->name}}</a></li>
            @endif
            <li class="breadcrumb-item active"><a href="{{route('plugin.index')}}#/{{$item->category->slug}}">{{$item->category->name}}</a></li>
        </ol>
        ...
        <x-front.reviewForm type="plugin" :id="$item->id"></x-front.reviewForm>
...
@section('script')
    ...
    <script>var type = "plugin", model_id="{{$item->id}}"; </script>
    <script src="{{asset('assets/js/front/plugin/detail.js')}}"></script>
@endsection
