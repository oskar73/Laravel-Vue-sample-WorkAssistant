...
@section('title', 'Bizinabox Service - ' . $item->name)

@section('seo')
    <link rel="canonical" href="{{ config('app.url') }}/services/{{ $item->slug }}">
@endsection
...
@endsection
@section('content')
    <x-front.hero>Service</x-front.hero>

    <div class="container">
        <ol class="breadcrumb mt-3">
            <li class="breadcrumb-item"><a href="{{route('service.index')}}">Service</a></li>
            @if($item->category->category->name!==null)
                <li class="breadcrumb-item"><a href="{{route('service.index')}}#/{{$item->category->category->slug}}">{{$item->category->category->name}}</a></li>
            @endif
            <li class="breadcrumb-item active"><a href="{{route('service.index')}}#/{{$item->category->slug}}">{{$item->category->name}}</a></li>
        </ol>
            ...
            <div class="col-md-7">
                <div class="product-detail">
                   ...
                   <div class="margin-20px-bottom white-space-pre-line">
                    ...
                    <div class="margin-20px-bottom">
                        @if($item->slashed_price!==null)<span class="margin-15px-right font-size18 font-weight-600 offer-price">${{formatNumber($item->slashed_price)}}</span> @endif
                        <span class="font-size18 font-weight-700 text-theme-color">${{formatNumber($item->price)}}</span>
                    </div>
                    <div class="row">
                    ...
                    <div class="row margin-20px-bottom">
                        <div class="col-lg-12">
                            ...
                            <button type="button" class="btn style-1 text-uppercase addToCartBtn" data-cart="1"><span> Buy Now</span></button>
                        </div>
                    </div>
        ...
        <x-front.reviewForm type="service" :id="$item->id"></x-front.reviewForm>
...
@section('script')
    ...
    <script>var type = "service", model_id="{{$item->id}}"; </script>
    <script src="{{asset('assets/js/front/service/detail.js')}}"></script>
@endsection
