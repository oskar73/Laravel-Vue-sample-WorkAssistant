...
class WebsiteAppointmentBlockDate extends BaseModel
{
    ...
    public function storeItem($request, $type, $product_id)
    {
        ...
        do {
            $blockHour = WebsiteAppointmentBlockDate::where("type", $type)
                ->where("product_id", $product_id)
                ->where('date', $start_date)
                ->first();
            if ($blockHour == null) {
                $blockHour = new WebsiteAppointmentBlockDate();
                $blockHour->type = $type;
                $blockHour->product_id = $product_id;
                $blockHour->date = $start_date;
                $blockHour->web_id = $web_id;
            }
