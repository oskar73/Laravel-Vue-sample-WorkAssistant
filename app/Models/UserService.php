...
class UserService extends BaseModel
{
    ...
    public function storeItemFromPurchase($item, $user, $orderItem)
    {
        for ($k = 0; $k < $item['quantity']; $k++) {
            ...
            $service->order_item_id = $orderItem->id;
            ...
            if ($item['item']['meeting'] == 1 && $item['item']->meetingSet()->exists()) {
                ...
                $userMeeting = $meeting->createUserMeeting($service, $item['item'], $user);
            ...
            if ($item['item']['form'] == 1 && $item['item']->getForm()->exists()) {
                $form = new UserForm();
                $form->createUserForm($service, $item['item'], $user);
    ...
    public function getDatatable($status, $user)
    {
        ...
        return Datatables::of($result)->addColumn('user', function ($row) {
            ...
            return '<a href="'.route("admin.purchase.service.detail", $row->id).'" class="btn btn-outline-info btn-sm m-1	p-2 m-btn m-btn--icon editBtn" data-action="detail">
                        <span>
                            <i class="la la-eye"></i>
                            <span>Detail</span>
                        </span>
