...
class Plugin extends BaseModel implements HasMedia
{
    ...
    {
        return $this->belongsTo(PluginCategory::class, 'category_id')->withDefault();
    }
    ...
    {
        try {
            $title = '<li class="breadcrumb-item"><a href="#/all">Plugins</a></li>';

            $category = null;
            if ($request->keyword == '') {
                if ($request->category === 'all') {
                    $filter_ids = PluginCategory::where(function ($query) {
                    ...
                    $query = Plugin::whereIn("category_id", $filter_ids);
                } else {
                    $category = PluginCategory::where('slug', $request->category)->first();

                    if ($category->parent_id === 0) {
                        $category_ids = $category->approvedSubCategories()->pluck("id")->toArray();
                        array_push($category_ids, $category->id);

                        $query = Plugin::whereIn("category_id", $category_ids);

                        $title .= "<li class='breadcrumb-item'><a href='#/{$category->slug}'>" . $category->name . "</a></li>";
                    } else {
                        $query = Plugin::where("category_id", $category->id);
                ...
                $query = Plugin::where(function ($query) use ($keyword) {
            ...
            $query2 = $query->where('plugins.status', 1)
                ->select('plugins.id', 'plugins.name', 'plugins.price', 'plugins.slashed_price', 'plugins.slug', 'plugins.featured', 'plugins.new', 'plugins.description')
                ->with('media');
            ...
            $items = $items->orderBy('plugins.created_at', 'DESC')->paginate(9);
            $view = view('components.front.pluginItem', compact('items', 'title', 'category'))->render();
