...
class WebsiteAvailableWeekday extends BaseModel
{
    ...
    public function storeItem($request, $type, $id = null)
    {
        foreach (self::weekArray as $item) {
            if ($request != null && $request->$item) {
                ...
                $end_time = 'end_time_'.$item;
                $weekday->$item = 1;
                WebsiteAvailableHour::where("weekday_id", $weekday->id)->delete();
                if ($request->$start_time != null) {
                    foreach ($request->$start_time as $key => $start) {
    ...
    {
        return $this->hasMany(WebsiteAvailableHour::class, "weekday_id");
    }
}
