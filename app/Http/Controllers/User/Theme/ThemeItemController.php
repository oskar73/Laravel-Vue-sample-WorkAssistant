...
class ThemeItemController extends UserController
{
    ...
    public function index()
    {
        if (request()->wantsJson()) {
            $themes = $this->model->where('user_id', auth()->user()->id)->get();

            $activeThemes = $themes->where('status', 1);
            $inactiveThemes = $themes->where('status', 0);

            $all = view('components.user.themeItem', [
                'themes' => $themes,
                'selector' => 'datatable-all',
            ])->render();

            $active = view('components.user.themeItem', [
                'themes' => $activeThemes,
                'selector' => 'datatable-active',
            ])->render();

            $inactive = view('components.user.themeItem', [
    ...
    public function store(Request $request)
    {
        try {
            ...
            $data = $request->all();
            $data['name'] = $this->model->getUniqueValue('name', $data['name'], ['user_id' => $request->user()->id]);
    ...
    public function update(Request $request)
    {
        try {
            ...
            $data['user_id'] = $request->user()->id; // for admin, user_id is null
            $theme->update($data);
